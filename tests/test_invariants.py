...
class TestR2Reproducibility:
    ...
    def test_with_controls_joint_path_r2_matches_raw_ssr_sst(self):
        ...
        result = RollingOLS(window=18, min_periods=12, mode="joint", dtype="float64").fit_transform(
...
class TestDtypePrecision:
    def test_float32_matches_float64_within_tolerance(self):
        ...
        for factor in factors.columns:
            beta_32 = result_32.get_beta(factor)
            beta_64 = result_64.get_beta(factor)
            both_valid = beta_32.notna() & beta_64.notna()
            np.testing.assert_allclose(
                beta_32.to_numpy()[both_valid.to_numpy()],
                beta_64.to_numpy()[both_valid.to_numpy()],
                rtol=0,
                atol=1e-4,
            )

    def test_output_dtype_is_always_float64_regardless_of_storage_dtype(self):
        """Internals run in float64 (per README); output frames must not
